...
echo "We are using an alternative rm remove script for archiving."
echo "The Original rm is in /bin/rm.old"
echo
mydir="$HOME/.deleted-files"
realrm="/bin/rm.old"
copy="/bin/cp -R"
echo "DELETED FILE ARE FOUND in '$mydir'.!!!"
if [ $# -eq 0 ]; then #let the 'rm' output the usage error
...
# Make sure that the $mydir exist
if [ ! -d $mydir ]; then
	if [ ! -w $HOME ]; then
...
for arg
do
	newname="$mydir/$(basename "$arg")".$(date "+%N.%d-%m-%Y")
	if [ -f "$arg" ]; then
		$copy "$arg" "$newname"
	elif [ -d "$arg" ]; then
		$copy "$arg" "$newname"
