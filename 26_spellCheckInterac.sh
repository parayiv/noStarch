...
# This can go as deep as the user might need, but keeping track of nesting
# enables
# us to ensure that only level 1 outputs the "replacing word" message.
	...
	do
		...
		echo -n "ignore, q)uit, or type replacement: "
		read fix
		if [ "$fix" = "q" -o "$fix" = "quit" ] ; then
			...
			# It's a single-word replacement, let's spell-check the replacement too
				if [ ! -z "$(echo $fix | $spell)" ] ; then
					echo ""
					echo "*** Your suggested replacement
					$fix is misspelled."
					echo "*** Preface the word with '!' to
					force acceptance."
				else
